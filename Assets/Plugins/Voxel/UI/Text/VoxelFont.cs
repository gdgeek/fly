...
namespace GDGeek{
	public class VoxelFont: MonoBehaviour {
		...
		private void rebuilt(){
			...
			_renderer.material.SetInt ("_Width", ci.maxX - 	ci.minX);
			_renderer.material.SetInt ("_Height", ci.maxY - ci.minY);
			_renderer.material.SetVector ("_Size", new Vector4 (ci.maxX, ci.minX, ci.maxY, ci.minY) );
			_renderer.material.SetVector ("_FontUV", new Vector4 (ci.uvTopLeft.x, ci.uvTopLeft.y, ci.uvTopRight.x, ci.uvTopRight.y));
			_renderer.material.SetVector ("_FontUV2", new Vector4 (ci.uvBottomLeft.x, ci.uvBottomLeft.y,  ci.uvBottomRight.x, ci.uvBottomRight.y));
			_renderer.material.SetColor ("_FontColor", _fontColor);
			_renderer.material.SetColor ("_Color", _bgColor);
		}
